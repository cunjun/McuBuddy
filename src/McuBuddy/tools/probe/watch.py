...
def set_watchpoint(
    ...
    if blocked := require_tool_confirmation("probe_set_watchpoint", confirm):
        return blocked
    return session.services.probe.set_watchpoint(address, size, watch_type)
...
def remove_watchpoint(session: SessionState, address: int, confirm: bool = False) -> dict:
    if blocked := require_tool_confirmation("probe_remove_watchpoint", confirm):
        return blocked
    return session.services.probe.remove_watchpoint(address)
...
def clear_all_watchpoints(session: SessionState, confirm: bool = False) -> dict:
    if blocked := require_tool_confirmation("probe_clear_all_watchpoints", confirm):
        return blocked
    return session.services.probe.clear_all_watchpoints()


def read_fpu_registers(session: SessionState) -> dict:
    if not probe_supports(session.services.probe, ProbeCapability.FPU_REGISTERS):
        return {
            "status": "error",
...
def read_cycle_counter(session: SessionState, confirm: bool = False) -> dict:
    if not probe_supports(session.services.probe, ProbeCapability.DWT_CYCLE_COUNTER):
        return {
            "status": "error",
    ...
    if blocked := require_tool_confirmation("read_cycle_counter", confirm):
        return blocked
    try:
        return session.services.probe.read_cycle_counter()
    except NotImplementedError:
        return {
            "status": "error",
            "summary": "Active probe backend does not support DWT cycle counter reads.",
...
def read_swo_log(
    ...
    if not probe_supports(session.services.probe, ProbeCapability.SWO):
        return {
            "status": "error",
    ...
    if blocked := require_tool_confirmation("read_swo_log", confirm):
        return blocked
    try:
        return session.services.probe.read_swo_log(
...
def read_itm_trace(
    ...
    if not probe_supports(session.services.probe, ProbeCapability.ITM_TRACE):
        return {
            "status": "error",
            "summary": "Active probe backend does not support ITM trace reads.",
        }
    try:
        return session.services.probe.read_itm_trace(
            cpu_speed_hz=cpu_speed_hz,
            swo_speed_hz=swo_speed_hz,
...
def read_mpu_regions(session: SessionState, confirm: bool = False) -> dict:
    ...
    for index in range(dregion):
        session.services.probe.write_memory(_MPU_RNR, index.to_bytes(4, "little"))
        rbar = int.from_bytes(session.services.probe.read_memory(_MPU_RBAR, 4), "little")
        rasr = int.from_bytes(session.services.probe.read_memory(_MPU_RASR, 4), "little")
