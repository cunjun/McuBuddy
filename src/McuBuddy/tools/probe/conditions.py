...
def _evaluate_condition(session: SessionState, cond: dict) -> bool:
    ...
    if cond.get("condition_symbol"):
        ...
        resolved = session.services.elf.resolve_symbol(cond["condition_symbol"])
        if resolved.get("address") is None:
            return True
        raw = session.services.probe.read_memory(int(resolved["address"], 16), 4)
        observed = int.from_bytes(raw, "little")
    elif cond.get("condition_register"):
        registers = session.services.probe.read_core_registers()
